...
impl EditLine {
    pub(crate) fn new(value: impl Into<SharedString>, cx: &mut Context<Self>) -> Self {
    ...
    pub(crate) fn new_password(value: impl Into<SharedString>, cx: &mut Context<Self>) -> Self {
        let mut input = Self::new(value, cx);
        input.masked = true;
        input
    }
...
impl EntityInputHandler for EditLine {
    fn text_for_range(
        &mut self,
...
impl Render for EditLine {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .min_w_0()
            .w_full()
            .child(EditLineElement { input: cx.entity() })
    }
}

impl Focusable for EditLine {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

struct EditLineElement {
...
impl IntoElement for EditLineElement {
    type Element = Self;
...
impl Element for EditLineElement {
    ...
    fn prepaint(
        ...
        let scroll_x = adjusted_scroll_x(
            previous_scroll_x,
            line.width(),
            bounds.size.width,
            cursor_pos,
        ...
        let (selection, cursor) = if selected_range.is_empty() {
            (
                ...
                Some(fill(
                    Bounds::new(
                        point(bounds.left() + cursor_pos - scroll_x, bounds.top() + px(2.)),
...
fn adjusted_scroll_x(
    ...
    let padding = px(4.);
    if line_width.as_f32() <= visible_width.as_f32() {
        return Pixels::ZERO;
    }

    let mut scroll_x = previous;
    let right_edge = scroll_x + visible_width - padding;
    if cursor_x.as_f32() > right_edge.as_f32() {
        scroll_x = cursor_x - visible_width + padding;
    }

    let left_edge = scroll_x + padding;
    if cursor_x.as_f32() < left_edge.as_f32() {
        scroll_x = cursor_x - padding;
    }

    let max_scroll = line_width - visible_width + padding;
    if scroll_x.as_f32() < 0. {
        Pixels::ZERO
    } else if scroll_x.as_f32() > max_scroll.as_f32() {
        max_scroll
    } else {
        scroll_x
    }
...
fn snap_to_char_boundary(value: &str, mut cursor: usize) -> usize {
