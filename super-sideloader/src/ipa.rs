use crate::models::{
    AppEntitlement, AppMetadata, AppOption, EntitlementValue, PatchOption, SupportedDeviceFamily,
};
...
pub(crate) fn read_ipa(path: &Path, patches: Vec<PatchOption>) -> Result<AppOption, String> {
    tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|error| format!("Failed to start IPA reader runtime: {error}"))?
        .block_on(read_ipa_async(path, patches))
}

async fn read_ipa_async(path: &Path, patches: Vec<PatchOption>) -> Result<AppOption, String> {
    let reader = ZipFileReader::new(path)
        .await
        .map_err(|error| format!("Failed to read IPA archive at {}: {error}", path.display()))?;
    ...
    let info_index = reader
        ...
        .ok_or_else(|| "IPA does not contain Payload/*.app/Info.plist.".to_string())?;
    let info_path = entry_name(&reader.file().entries()[info_index])
        .ok_or_else(|| "IPA Info.plist entry name is invalid.".to_string())?;
    let app_prefix = info_path
        .strip_suffix("Info.plist")
        .ok_or_else(|| "IPA Info.plist entry path is invalid.".to_string())?
        .to_string();

    let info_bytes = read_entry(&reader, info_index, "Info.plist").await?;
    let info = plist::from_bytes::<Dictionary>(&info_bytes)
        .map_err(|error| format!("Failed to parse IPA Info.plist: {error}"))?;
    ...
    let entitlements = extract_entitlements(&reader, &app_prefix)
        .await
        .unwrap_or_else(|| fallback_entitlements(bundle_id));
    ...
    Ok(AppOption {
        ...
        icon_path: icon_path.map(|path| path.to_string_lossy().to_string().into()),
...
fn required_plist_string<'a>(info: &'a Dictionary, key: &str) -> Result<&'a str, String> {
    plist_string(info, key).ok_or_else(|| format!("IPA Info.plist is missing string key {key}."))
}

async fn read_entry(reader: &ZipFileReader, index: usize, label: &str) -> Result<Vec<u8>, String> {
    let mut entry_reader = reader
        .reader_without_entry(index)
        .await
        .map_err(|error| format!("Failed to read {label} from IPA: {error}"))?;
    let mut data = Vec::new();
    entry_reader
        .read_to_end(&mut data)
        .await
        .map_err(|error| format!("Failed to extract {label} from IPA: {error}"))?;
    Ok(data)
}
...
async fn extract_icon(
    ...
    let Some((index, name)) = fallback else {
        return None;
    };
...
fn entitlement_value(value: &Value) -> EntitlementValue {
    match value {
        Value::String(value) => EntitlementValue::String(value.clone().into()),
        Value::Boolean(value) => EntitlementValue::Boolean(*value),
        Value::Integer(value) => value
            .as_signed()
            .map(EntitlementValue::Integer)
            .unwrap_or_else(|| EntitlementValue::String(value.to_string().into())),
        ...
        Value::Dictionary(values) => EntitlementValue::Dictionary(
            values
                .iter()
                .map(|(key, value)| (key.clone().into(), entitlement_value(value)))
                .collect(),
        ),
        Value::Data(value) => EntitlementValue::Data(value.clone()),
        Value::Date(value) => EntitlementValue::Date(value.to_xml_format().into()),
        Value::Uid(value) => EntitlementValue::Uid(value.get()),
        _ => EntitlementValue::Unknown("Value".into()),
    }
}

fn fallback_entitlements(bundle_id: &str) -> Vec<AppEntitlement> {
    vec![
        AppEntitlement {
            key: "application-identifier".into(),
            value: EntitlementValue::String(format!("TEAMID.{bundle_id}").into()),
        },
        AppEntitlement {
